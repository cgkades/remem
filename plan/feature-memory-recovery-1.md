...
1. **Evidence recording scope (Phase 2, TASK-007):** record all raw evidence from the start once
   configured -- both `direct-user` and host-observed `assistant`/`tool` origins together, not a
   separately-gated assistant/tool toggle. Recording evidence is not the same as promoting it to a
   fact (that remains Phase 6's separate, still-partially-gated policy, decision 3 below). The
   other TASK-007 development-profile values (disabled until configured; project scope only; 8
   KiB/16 references/32 queued events; unknown origins cannot auto-promote) are approved as
   proposed.
2. **Missing-identity handling (Phase 2, TASK-007/TASK-008):** confirmed as proposed -- skip
   storing the event's content rather than deduplicating by guessed/text-derived identity -- with
   one addition: the rejection must be logged as a bounded diagnostic (reason code and a count,
   e.g. `unsupported-identity, host=X, count=N`), never silently discarded and never including the
   rejected content, matching the existing capture `explain()` diagnostic pattern.
3. **Fact-promotion boundary (Phase 6, TASK-022 and new TASK-056):** a host-verified action with an
   observable outcome (a file was written, a command executed, with a checkable result) may
   promote to a **generalized** fact describing what was done, if not corrected -- not the raw
   diff/output, and not an added causal/root-cause claim beyond what was actually verified. An
   unverified explanation, a research/causal claim, or AI output responding to a question or
   decision that needed the user's input, stays episodic-only if the user simply moves on;
   "moved on" is never sufficient by itself for that class. It only becomes a fact through the
   existing explicit-user-confirmation capture path (including a short reply like "ok, do it" --
   that is an explicit statement, not silence, and needs no new mechanism) or through independent
   verification. A correction always supersedes via the existing correction workflow.
4. **Entity/relationship linking (Phase 6/8, new TASK-055/TASK-057):** `remem.entities`,
   `remem.memory_entities`, `remem.relationships`, and the `MemoryEntity`/`MemoryRelationship`
   fields on `MemoryWrite`/`MemoryRecord` already exist in the schema and are already returned by
   point-reads; extraction should start populating them (e.g., linking a "wrote file X" fact to the
   bug/feature entity it happened under) and `search()`/synthesis should start surfacing them,
   rather than treating this as a new schema design problem.
5. **Bounded temporal/episode recall (Phase 9, new TASK-058):** add a deterministic, explicitly
   user-triggered recall mode analogous to Phase 1's continuity-anchor trigger -- never silently
   expanding ordinary relevance search. Start with a **fixed, bounded phrase list** (e.g. `today`,
   `yesterday`, `this morning`, `this afternoon`, `last night`, `last week`, `last <weekday>`), not
...
Host identity must be documented or use an established stable turn identity. Missing both yields an explicit unsupported-identity result for the new evidence path; do not deduplicate by prompt text. **Approved 2026-09-10:** this rejection must be logged as a bounded diagnostic (reason code plus a count, e.g. `unsupported-identity, host=X, count=N`) rather than silently discarded; never log the rejected content itself. Reject NUL-containing identity fields and bound each to 256 UTF-16 code units. Preserve legacy IDs. A new namespace must be versioned and include provider/host/project/session identity. Hash canonical sanitized immutable evidence, excluding ingestion time and later classifier results. Same identity/different evidence is a collision, not an upsert.

**Development profile, approved by the maintainer on 2026-09-10 (see "Maintainer Decisions" in §1), not a shipped-default change:** new evidence capture is disabled until configured. Once configured: project scope only; `direct-user` and host-observed `assistant`/`tool` origins are captured **together** from the start (recording evidence is not the same as promoting it to a fact -- see Phase 6's approved fact-promotion boundary). Maximum serialized safe event payload 8 KiB, 16 evidence references, and 32 queued events, approved as initial bounded limits -- revisit if real usage shows they are too small. Unknown origins cannot auto-promote. Reuse configured timeouts and bounded shutdown.

| Task     | Description                                                                                                                                                                                                                                                                                                        | Completed | Date          |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ------------- |
| TASK-007 | Define the additive normalized-envelope/admission-result contracts, source enablement, and identity rules **per the approved contract above**. Preserve old `SessionObservation` consumers.                                                                                                                        | No        | Not completed |
...
**Approved fact-promotion boundary (2026-09-10, see "Maintainer Decisions" in §1):** a host-verified action with an observable outcome (a file was written, a command ran, with a checkable result) may promote to a **generalized** fact describing what was done -- e.g. "wrote `src/foo.ts` to forward the credential provider chain" -- not the raw diff/output, and not an added causal/root-cause claim beyond what was actually verified, if the action is not corrected. An unverified explanation, research/causal claim, or AI output responding to a question or decision that needed the user's input stays episodic-only when the user simply moves on; "moved on" is never sufficient by itself for that class. It becomes a fact only through the existing explicit-user-confirmation capture path (a short reply like "ok, do it" is an explicit statement, not silence -- no new mechanism needed for that case) or through independent verification. A correction always supersedes via the existing correction workflow, never this policy.
...
| TASK-022 | Implement the pure ordered learning policy and all four outcomes, including the approved fact-promotion boundary above. Test disabled auto-promotion, missing/foreign/expired evidence, false success, conflicts, high-impact institutional data, scope escalation, unknown provenance, a corrected action, and an unverified causal claim the user did not object to. Rejection must not remove safe source episodes.                                                                                                                                                | No        | Not completed |
...
| TASK-055 | New 2026-09-10: populate the existing `remem.entities`/`remem.memory_entities`/`remem.relationships` tables and `MemoryEntity`/`MemoryRelationship` fields on `MemoryWrite` (`src/types.ts`) during evidence-linked extraction -- link an action/fact to the entity (bug/feature/file) it happened under, using only admitted evidence already in scope. Do not invent a new table; this schema and the write-path support for it already exist. Derive entity names/aliases from admitted evidence, not arbitrary generated names, consistent with TASK-027's bound. | No        | Not completed |
| TASK-056 | New 2026-09-10: implement and test the approved fact-promotion boundary above as its own regression matrix, independent of TASK-022's broader policy tests: a host-verified action not corrected becomes a generalized fact; the same action's added but unverified causal claim does not; an explanation/question left un-responded-to stays episodic-only; a short explicit confirmation ("ok, do it") promotes through the existing explicit-capture path, not a new one.                                                                                          | No        | Not completed |
...
| TASK-057 | New 2026-09-10: extend `search()`'s SQL (`src/providers/postgres.ts`) to also return `entities`/`relationships`, matching what point-reads (`BASE_SELECT`) already return -- this is a query-shape addition, not a new schema. Extend synthesis to mention linked memories (TASK-055) within existing token budgets, attributed the same as any other result; do not query unrelated history unconditionally. | No        | Not completed |
...
| TASK-058 | New 2026-09-10, approved bounded scope (see "Maintainer Decisions" in §1): add a deterministic, explicitly-triggered temporal-recall mode, structured like Phase 1's continuity-anchor trigger. Trigger 1 recognizes a **fixed, bounded phrase list only** (`today`, `yesterday`, `this morning`, `this afternoon`, `last night`, `last week`, `last <weekday>`) and, only then, issues a time-range-filtered request over `observedAt`/`createdAt` instead of ordinary relevance search; open-ended date parsing (e.g. "around two weeks ago") is explicitly out of scope until real usage justifies expanding the list. Trigger 2 recognizes an entity-scoped request ("what else did we do while fixing this/that") and reuses the Phase 1 anchor match plus TASK-055/TASK-057's relationship links to list other memories connected to the same entity, independent of any timestamp. Neither trigger fires on ordinary prompts; test both positive triggers and discriminating negative controls (untriggered phrasing must behave exactly as today). | No        | Not completed |
