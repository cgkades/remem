...
export async function runCli(args: string[], dependencies: CliDependencies = {}): Promise<number> {
  ...
  // Routed through the same errorOutput/exit-1 convention as every other
  // command below (rather than left to propagate as an unhandled rejection)
  // if package.json is missing/malformed -- e.g. a corrupted install.
  if (args.length === 1 && (args[0] === "--version" || args[0] === "-V")) {
    try {
      ...
      errorOutput(
        `Remem --version failed: ${error instanceof Error ? error.message : "unknown error"}`,
      )
      return 1
    }
  }
